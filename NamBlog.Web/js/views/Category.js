...
export default {
    ...
    setup() {
        ...
        const fetchArticlesByCategory = async (category) => {
            ...
            const query = `
                query GetArticlesByCategory($page: Int!, $pageSize: Int!, $category: String!, $isPublished: Boolean) {
                    blog {
                        article {
                            articles(page: $page, pageSize: $pageSize, category: $category, isPublished: $isPublished) {
                                items {
                                    id
                                    title
                                    slug
        ...
        watch(() => route.params.category, (newCategory) => {
            if (newCategory) {
                fetchArticlesByCategory(newCategory);
            }
        });
    ...
    template: `
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
            ...
            <div :class="[
                ...
                <!-- Category Header -->
                <div class="mb-8">
                    <h1 class="text-3xl font-bold text-gray-800 dark:text-gray-100">{{ categoryName }}</h1>
                ...
                <div v-else-if="articles.length === 0" class="text-center py-10 text-gray-500 dark:text-gray-400">
                    {{ t('article.noArticlesInCategory') }}
                </div>
