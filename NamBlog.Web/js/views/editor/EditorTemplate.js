...
<div class="w-full px-4 py-8 flex flex-col flex-1 min-h-0 relative">
    ...
    <div class="flex flex-col flex-1 min-h-0">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <input v-model="form.title" :placeholder="t('editor.titlePlaceholder')"
            ...
            <input v-model="form.slug" :placeholder="t('editor.slugPlaceholder')"
        ...
        <div class="flex flex-wrap items-center gap-3 mb-6">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <label class="flex items-center gap-2 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 shadow-sm hover:shadow-md transition-all cursor-pointer">
                        <span class="text-sm font-medium text-gray-900 dark:text-gray-300 whitespace-nowrap">📢 {{ t('editor.publish') }}</span>
                    ...
                    <!-- 收藏开关 -->
                    <label class="flex items-center gap-2 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 shadow-sm hover:shadow-md transition-all cursor-pointer">
                        <span class="text-sm font-medium text-gray-900 dark:text-gray-300 whitespace-nowrap">⭐ {{ t('editor.featured') }}</span>
            ...
            <div class="flex items-center gap-3">
                ...
                <button @click="clearDraft"
                        class="px-3 py-2 text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200 transition-all hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg border border-gray-300 dark:border-gray-600 disabled:opacity-50 disabled:cursor-not-allowed"
                        :disabled="isBusy"
                        :title="t('editor.clearDraftTitle')">
                    🗑️ {{ t('editor.clearDraft') }}
                </button>
                ...
                <button @click="saveMetadata"
                        ...
                        :disabled="isBusy"
                        :title="isNew ? t('editor.firstSaveHint') : t('editor.saveMetaHint')">
                    {{ isSavingMeta ? (isNew ? '⌛ ' + t('editor.savingFirst') : '⌛ ' + t('editor.saving')) : '💾 ' + t('editor.saveArticle') }}
                </button>
            </div>
        </div>
            ...
            <div class="flex items-start gap-3 pb-2 overflow-x-auto editor-scrollbar" ref="promptListRef">
                ...
                <div ref="customPromptRef" @click.self="handleCustomPromptClick"
                    ...
                    <div class="flex justify-between items-center mb-2" @click.stop="toggleCustomPromptExpand">
                        <div class="flex items-center gap-2 cursor-pointer">
                            <span class="text-sm font-semibold" :class="selectedPromptIndex === -1 ? 'text-green-700 dark:text-green-300' : 'text-gray-700 dark:text-gray-300'">✏️ {{ t('editor.customPrompt') }}</span>
                            <span v-if="selectedPromptIndex === -1" class="text-xs px-2 py-0.5 bg-green-200 dark:bg-green-800 rounded-full">{{ t('editor.inUse') }}</span>
                        </div>
                    ...
                    <div v-if="isCustomPromptExpanded" class="relative">
                        <textarea v-model="form.customPrompt"
                                  ...
                                  class="w-full p-3 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none resize-none shadow-inner editor-scrollbar"
                                  style="word-break: break-word; white-space: pre-wrap; height: 300px;"></textarea>
                        <!-- 全屏编辑按钮 -->
                    ...
                    <div v-else class="text-sm line-clamp-3 overflow-hidden"
                        ...
                        {{ form.customPrompt || t('editor.customPromptEmpty') }}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mb-6" v-else>
            <label class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2 flex items-center gap-2">
                ...
                {{ t('editor.customPromptOptional') }}
            </label>
            <textarea v-model="form.customPrompt"
                      :placeholder="t('editor.customPromptNew')"
                ...
                <div class="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-gray-700 dark:to-gray-700 px-5 py-3 border-b border-gray-200 dark:border-gray-600 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <button v-if="!isNew" @click="deleteArticle"
                                ...
                                :disabled="isBusy"
                                :title="versions.length <= 1 ? t('editor.deleteVersionHint') : t('editor.deleteCurrentVersion')">
                            {{ isDeleting ? t('editor.deleting') : '🗑️ ' + t('editor.deleteVersion') }}
                        </button>
                        <div class="flex items-center gap-2">
                            <svg class="w-5 h-5 text-green-500" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M3 5a2 2 0 012-2h10a2 2 0 012 2v8a2 2 0 01-2 2h-2.22l.123.489.804.804A1 1 0 0113 18H7a1 1 0 01-.707-1.707l.804-.804L7.22 15H5a2 2 0 01-2-2V5zm5.771 7H5V5h10v7H8.771z" clip-rule="evenodd"/>
                            </svg>
                            <span class="font-semibold text-sm text-gray-700 dark:text-gray-200">{{ t('editor.htmlPreview') }}</span>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        <!-- 全屏预览/编辑按钮 -->
                        ...
                        <button @click="htmlContent ? clearHtml() : generateHtml()"
                            ...
                            {{ isGenerating ? '⌛ ' + t('editor.generating') : (htmlContent ? '🧹 ' + t('editor.clearHtml') : '✨ ' + t('editor.generateHtml')) }}
                        </button>
                    </div>
                </div>
                <div class="flex-1 relative overflow-hidden bg-white dark:bg-gray-900">
                    ...
                    <div v-if="isGenerating" class="absolute top-0 left-0 right-0 z-10">
                        ...
                        <div class="bg-gradient-to-r from-blue-50 to-purple-50 dark:bg-gray-800 p-4 text-center">
                            <span class="text-sm font-medium bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent animate-pulse">✨ {{ t('editor.generatingProgress', { progress: generationProgress }) }}</span>
                        </div>
                    </div>
                    ...
                    <iframe
                        ...
                        :title="t('editor.htmlPreviewTitle')">
                    </iframe>
                    <div v-show="!htmlContent && !isGenerating" class="flex flex-col items-center justify-center h-full text-gray-400 dark:text-gray-500">
                        ...
                        <p class="text-sm">{{ t('editor.clickToPreview') }}</p>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-gray-700 dark:to-gray-700 px-5 py-3 border-b border-gray-200 dark:border-gray-600 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <button @click="toggleHtmlPreview"
                                ...
                                :title="isHtmlCollapsed ? t('editor.expandHtmlPreview') : t('editor.collapseHtmlPreview')">
                        ...
                        <span class="font-semibold text-sm text-gray-700 dark:text-gray-200">{{ t('editor.markdownSource') }}</span>
                    </div>
                    <!-- 创建新版本按钮 -->
                    <button @click="submitArticle"
                            ...
                            :disabled="isBusy"
                            :title="htmlContent ? t('editor.createVersionWithHtml') : t('editor.createVersionWithAI')">
                        {{ isSubmitting ? t('editor.submitting') : (isNew ? t('editor.createArticleButton') : t('editor.createNewVersionButton')) }}
                    </button>
                </div>
            ...
            <div class="w-full h-full max-w-4xl flex flex-col bg-white dark:bg-gray-800 rounded-xl shadow-2xl overflow-hidden">
                ...
                <div class="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-gray-700 dark:to-gray-700 border-b border-gray-200 dark:border-gray-600">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-gray-800 dark:text-gray-200 font-semibold">✏️ {{ t('editor.editPrompt') }}</span>
                    </div>
        ...
        <transition name="slide-left">
                ...
                <div class="flex items-center justify-between gap-3">
                    ...
                    <div class="flex items-center gap-3 flex-wrap">
                        <div class="flex items-center gap-2 flex-shrink-0">
                            ...
                            <span class="font-semibold text-gray-700 dark:text-gray-200">{{ t('editor.htmlPreview') }}</span>
                        </div>
            ...
            <div class="h-full overflow-hidden pb-16">
                <iframe
                    ...
                    :title="t('editor.htmlMobilePreviewTitle')">
                </iframe>
                <div v-else class="flex flex-col items-center justify-center h-full text-gray-400 dark:text-gray-500 p-4">
                    <svg class="w-16 h-16 mb-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5M7.188 2.239l.777 2.897M5.136 7.965l-2.898-.777M13.95 4.05l-2.122 2.122m-5.657 5.656l-2.12 2.122"/>
                    </svg>
                    <p class="text-sm text-center">{{ t('editor.noHtmlContent') }}<br/>{{ t('editor.pleaseGenerateFirst') }}</p>
                </div>
            </div>
        </div>
        </transition>
