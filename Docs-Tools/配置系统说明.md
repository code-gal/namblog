...
> 中文版本 | [English Version](Configuration-Guide.md)
...
**前端语言配置**：
- NamBlog 支持中英文双语界面，可通过前端配置文件自定义语言或创建自定义语言包
- 详见：[语言配置说明](语言配置说明.md) | [Language Configuration](Language-Configuration.md)
...
- **热重载**：✅ 支持，修改后立即生效
- **详细说明**：参见 [网站统计脚本配置说明](网站统计脚本配置说明.md) | [Analytics Configuration](Analytics-Configuration.md)
- **示例**：
  ```json
  {
    "Blog": {
...
支持在文章页导航面板分类列表下方显示自定义 HTML 内容（如二维码、广告、推荐链接等）：

- **配置项**：`Blog.ArticleSidebarWidget`
- **热重载**：✅ 支持，修改后立即生效
- **模板变量**：支持 `{{articleTitle}}`、`{{articleSlug}}`、`{{articleId}}`
- **深色模式**：自动适配，建议使用 rgba 和相对透明度
- **详细说明**：参见 [文章页侧边栏组件配置说明](文章页侧边栏组件配置说明.md) | [Article Sidebar Widget Configuration](Article-Sidebar-Widget-Configuration.md)
- **示例**：
  ```json
  {
    "Blog": {
      "ArticleSidebarWidget": "<div style='text-align:center; padding:16px; background:rgba(243,244,246,0.5); border-radius:8px;'><h4 style='font-size:14px; margin:0 0 12px;'>❤️ 赞赏支持</h4><img src='/images/donate.png' style='width:100px;' /><p style='font-size:12px; opacity:0.7; margin:8px 0 0;'>感谢您的支持</p></div>"
    }
  }
  ```
...
| 配置项 | 类型 | 说明 |
|-------|------|------|
| **BotUserAgents** | string[] | 爬虫 User-Agent 关键词列表（不区分大小写） |

**功能说明**：
- 当检测到列表中的 User-Agent 访问 `/article/{slug}` 时，自动重写到静态 HTML
- 支持搜索引擎爬虫（Google、Bing、Baidu 等）
- 支持 AI 爬虫（ChatGPT、Claude、Perplexity 等）
- 支持社交分享预览（Facebook、Twitter、LinkedIn 等）
- 配合 `/sitemap.xml` 和 `/robots.txt` 提升 SEO 效果
